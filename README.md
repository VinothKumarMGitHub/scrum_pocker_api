...
```bash
scrum_pocker_api$ sam build --use-container
```
...
```bash
scrum_pocker_api$ sam local invoke HelloWorldFunction --event events/event.json
```
...
```bash
scrum_pocker_api$ sam logs -n HelloWorldFunction --stack-name scrum_pocker_api --tail
```
...
```bash
scrum_pocker_api$ pip install pytest pytest-mock --user
scrum_pocker_api$ python -m pytest tests/ -v
```
...
```bash
aws cloudformation delete-stack --stack-name scrum_pocker_api
```
